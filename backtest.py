...
def run_backtest(returns : pd.DataFrame , rolling_window : int, rebalancing_freq : int, risk_free_rate : float):
    ...
    for i in range(rolling_window, len(returns), rebalancing_freq):
        ...
        historical_window = returns.iloc[i-rolling_window:i]
        ...
        future_returns = returns.iloc[i:i+rebalancing_freq].mean()
        # Start from i to prevent look-ahead bias
